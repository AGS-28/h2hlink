...
class Model_home extends CI_Model {
    ...
    {
        $sql = "SELECT a.status, COUNT(b.id) AS jml
    ...
    function get_data_draft_transaction()
    {
    ...
    function get_data_request_transaction()
    {
        $sql = "SELECT a.client_name, COUNT(b.id) AS jml, a.id
                FROM profile.clients a 
                LEFT JOIN trans.headers b ON b.client_id = a.id
                GROUP BY a.id
                ORDER BY a.client_name";
        $result = $this->db->query($sql);
        $arr_result = $result->result_array();

...
function get_data_all() {
    var url         = baseurl + "index.php/cms/get_data_file/"+Math.random();
...
{
    var url = siteurl + '/cms/add_item_file/'+Math.random();
    add(form,url);
}

function edit(id) 
{
    var url = siteurl + '/cms/get_edit_file/'+Math.random();
    ...
    if (respondData.status == 1) {
        $('#filetype').val(thisdata.message_type);
...
function hapus(id) {
    ...
    var url = siteurl + '/cms/hapus_file/'+Math.random();
    var postdata = new FormData();
    postdata.append('id',id);
    var data = post_ajax(url,postdata);
    var respondData = JSON.parse(data);
